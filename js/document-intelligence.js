 ...
 * AI-powered document processing and field extraction
 */
...
function openDocumentViewerModal(documentId) {
...
function showFilePreview(file) {
    const dropzone = document.querySelector('#dropzone .dropzone-content');
...
function clearFileSelection() {
    ...
    document.getElementById('upload-booking-number').value = '';
}
...
async function uploadDocument() {
    ...
    const bookingNumber = document.getElementById('upload-booking-number').value.trim();
    const autoCreate = document.getElementById('auto-create-transactions').checked;
    const createTasks = document.getElementById('create-review-tasks').checked;

    // Show processing state
    const uploadBtn = document.getElementById('upload-btn');
    uploadBtn.disabled = true;
    uploadBtn.innerHTML = '<i class="ri-loader-4-line processing-spinner"></i> Processing...';
    ...
    try {
        ...
        alert(`✓ Document uploaded! Processing started.`);
        ...
        uploadBtn.innerHTML = '<i class="ri-upload-cloud-line"></i> Upload & Process';
    }
}
...
async function processDocument(documentId, autoCreate = true, createTasks = true) {
    ...
    try {
        ...
        for (const pageData of pages) {
            ...
            if (page.isRelevant) {
                ...
                if (autoCreate && extractionResult.overallConfidence >= 0.85) {
                    await autoCreateTransaction(extractionResult);
                }
        ...
        window.localDB.events.create({
            ...
            description: `Processed ${document.fileName}: ${relevantPages} relevant pages, ${autoCreated} auto-created`,
            confidence: avgConfidence
        });
...
async function extractPages(document) {
    // Simulate page extraction from PDF
    // In real implementation, this would use PDF.js or similar

    const numPages = document.documentType === 'pod'
        ? Math.floor(Math.random() * 3) + 1  // 1-3 pages for POD
        : Math.floor(Math.random() * 5) + 2; // 2-6 pages for other docs

    const pages = [];
    for (let i = 0; i < numPages; i++) {
        const relevanceScore = i === 0 ? 0.95 : Math.random(); // First page usually relevant
...
async function performAIExtraction(document, page) {
    // Simulate AI extraction using Claude Vision API
    // In real implementation, this would call the Anthropic API

    await sleep(1000);

    // Simulate extracted fields with confidence scores
...
async function autoCreateTransaction(extractionResult) {
    try {
        ...
        if (!booking) {
            console.log('Booking not found for auto-creation:', extractionResult.bookingNumber);
            return;
        }

        // Get stops for booking
        ...
        // Create transaction at destination stop
        ...
        const transaction = window.localDB.transactions.create({
            ...
            notes: `Auto-created from document extraction (confidence: ${(extractionResult.overallConfidence * 100).toFixed(0)}%)`
        });

        // Mark extraction as auto-created
        ...
        window.localDB.events.create({
            ...
            performedBy: 'AI',
            source: 'document_intelligence',
            description: `Auto-created transaction from POD: ${extractionResult.extractedFields.quantity} ${extractionResult.extractedFields.equipmentType}`,
...
async function loadDocuments() {
    ...
    if (documents.length === 0) {
        container.innerHTML = `
            <div style="padding: 3rem; text-align: center; color: var(--text-tertiary);">
                ...
                <p style="font-size: 0.875rem; margin-top: 0.5rem;">Upload a POD or CMR to get started</p>
            </div>
        `;
        return;
    }
    ...
    container.innerHTML = sortedDocs.map(doc => {
        ...
        return `
            <div class="document-card" onclick="openDocumentViewerModal(${doc.id})">
...
function loadDocumentViewer() {
    ...
    // Render extraction results sidebar
...
function renderPageCanvas(page) {
    ...
    // In real implementation, this would render the PDF page or image
    canvas.innerHTML = `
...
function viewerZoomIn() {
    ...
    document.getElementById('viewer-zoom').textContent = currentZoom + '%';
...
function deleteDocument() {
    ...
    if (!confirm(`Delete ${currentDocument.fileName}? This cannot be undone.`)) {
        return;
    }
